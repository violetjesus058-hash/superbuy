 ...
 * 2. Replace [Superbuy Spreadsheet](docs.google.com) -> [Superbuy Spreadsheet](https://usfanslinki.com/)
...
function processArticle(filePath) {
  ...
  // Step 2: Check for existing Superbuy Spreadsheet anchor (case-insensitive)
  const ksRegex = /\[(superbuy spreadsheet)\]\(([^)]+)\)/gi;
  ...
  if (!hasKsAnchor) {
    ...
    const summaryNoLink = summary.replace(
      'visit the [Superbuy Spreadsheet](https://usfanslinki.com/) homepage',
      'visit the Superbuy Spreadsheet homepage'
    );
    content = content.trimEnd() + '\n\n' + summaryNoLink + '\n';
  }
